...
def index(request):
    #Query the database for a llist of ALL categories currently stored.
    #Order the categories by number of likes in descending order.
    #Retreve the top 5 only -- or all if less than 5.
...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if user_form.is_valid() and profile_form.is_valid():
            ...
            # Prints problems to the terminal.
            print(user_form.errors, profile_form.errors)
    else:
        # Not a HTTP POST, so we rrender our form using two ModelForm instances.
...
def user_login(request):
    ...
    if request.method == 'POST':
        ...
        # with matching credentials is found.
        if user:
